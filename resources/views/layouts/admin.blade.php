...
<body class="bg-slate-50 dark:bg-ink-950 text-ink-900 dark:text-ink-100 antialiased">
        ...
        <div class="w-full max-w-2xl bg-white dark:bg-ink-900 rounded-2xl shadow-float border border-slate-200 dark:border-ink-800 overflow-hidden animate-scale-in mx-4">
            ...
            <div id="cmdResults" class="max-h-96 overflow-y-auto p-2">
                ...
                <div class="cmd-section">
                    ...
                    <a href="{{ route('admin.datasets.index') }}" class="cmd-item flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
                    ...
                    <a href="{{ route('admin.posts.index') }}" class="cmd-item flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="cmd-item flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
                ...
                <div class="cmd-section mt-2">
                    ...
                    <a href="{{ route('profile') }}" class="cmd-item flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
                        <div class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-ink-800 flex items-center justify-center">
    ...
    <aside id="sidebar" class="sidebar bg-ink-900 dark:bg-ink-950 text-white sidebar-glow overflow-hidden border-r border-white/5 flex flex-col">
        ...
        <div class="px-4 py-4 border-b border-white/5 shrink-0">
            <div class="flex items-center gap-3 p-2 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                ...
                <div class="brand-text flex-1 min-w-0">
                    ...
                    <p class="text-[11px] text-white/50 truncate leading-tight mt-0.5">{{ ucfirst(auth()->user()->role ?? 'admin') }}</p>
                </div>
            </div>
        </div>
        @endauth
        ...
        <nav class="flex-1 overflow-y-auto sidebar-scroll px-3 py-4 space-y-6">
            ...
            <div>
                ...
                <a href="{{ route('admin.dashboard') }}" data-tooltip="Dashboard"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors {{ request()->routeIs('admin.dashboard') ? 'active text-white bg-white/10' : '' }}">
                    <i class="bi bi-grid-1x2-fill text-[17px] w-5 text-center"></i>
                    <span class="sidebar-label flex-1">Dashboard</span>
                </a>
                
                <a href="{{ route('admin.datasets.index') }}" data-tooltip="Datasets"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors {{ request()->routeIs('admin.datasets.*') ? 'active text-white bg-white/10' : '' }}">
                    ...
                    @if($pendingCount > 0)
                    <span class="sidebar-badge inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-md bg-gradient-to-r from-amber-400 to-orange-500 text-ink-900 shadow-sm">{{ $pendingCount }}</span>
                    @endif
                </a>
                
                <a href="{{ route('admin.users.index') }}" data-tooltip="Users"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors {{ request()->routeIs('admin.users.*') ? 'active text-white bg-white/10' : '' }}">
                    <i class="bi bi-people-fill text-[17px] w-5 text-center"></i>
                    <span class="sidebar-label flex-1">Users</span>
                </a>
                
                <a href="{{ route('admin.statistics') }}" data-tooltip="Analytics"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors {{ request()->routeIs('admin.statistics') ? 'active text-white bg-white/10' : '' }}">
            ...
            <div>
                ...
                <a href="{{ route('admin.posts.index') }}" data-tooltip="Posts"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors {{ request()->routeIs('admin.posts.*') ? 'active text-white bg-white/10' : '' }}">
                    ...
                    @if(class_exists('\App\Models\Post'))
                        ...
                        @if($postsCount > 0)
                        <span class="sidebar-badge inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-md bg-white/10 text-white/80">{{ $postsCount }}</span>
                        @endif
                    @endif
                </a>
                
                <a href="{{ route('admin.categories.index') }}" data-tooltip="Categories"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors {{ request()->routeIs('admin.categories.*') ? 'active text-white bg-white/10' : '' }}">
                    <i class="bi bi-tags-fill text-[17px] w-5 text-center"></i>
                    <span class="sidebar-label flex-1">Categories</span>
                </a>
                
                <a href="{{ route('datasets.index') }}" data-tooltip="Public Site"
            ...
            <div>
                ...
                <a href="{{ route('profile') }}" data-tooltip="Settings"
                   class="nav-item sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors">
        ...
        <div class="px-3 py-3 border-t border-white/5 shrink-0">
            ...
            <p class="sidebar-subtitle px-3 mt-3 text-[10px] text-white/30">© {{ date('Y') }} DataSphere v2.0</p>
        </div>
    </aside>
            ...
            <div class="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div class="hidden sm:block min-w-0">
                    ...
                    <div class="flex items-center gap-1.5 text-[11px] text-ink-500 mt-0.5">
                        ...
                        <span class="text-ink-700 dark:text-ink-300 font-medium truncate">@yield('page-title', 'Dashboard')</span>
                    </div>
                </div>
            </div>
                ...
                <div class="relative">
                    ...
                    <div id="notifDropdown" class="hidden absolute right-0 mt-2 w-80 bg-white dark:bg-ink-900 rounded-xl shadow-float border border-slate-200 dark:border-ink-800 overflow-hidden animate-scale-in origin-top-right">
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-ink-800 flex items-center justify-between">
                            ...
                            @if($pendingCount > 0)
                            <span class="text-[10px] font-bold text-brand-700 dark:text-brand-400 bg-brand-100 dark:bg-brand-900/30 px-2 py-0.5 rounded-full">{{ $pendingCount }} new</span>
                            @endif
                        </div>
                        <div class="max-h-80 overflow-y-auto">
                            @if($pendingCount > 0)
                            <a href="{{ route('admin.datasets.index', ['status' => 'pending']) }}" class="flex gap-3 p-4 hover:bg-slate-50 dark:hover:bg-ink-800/50 transition-colors border-b border-slate-100 dark:border-ink-800">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-ink-500 mt-0.5">{{ $pendingCount }} dataset(s) waiting for approval</p>
                        ...
                        <a href="{{ route('admin.datasets.index') }}" class="block px-4 py-2.5 text-center text-xs font-semibold text-brand-600 dark:text-brand-400 hover:bg-slate-50 dark:hover:bg-ink-800/50 transition-colors border-t border-slate-100 dark:border-ink-800">
                ...
                <div class="relative">
                    <button id="userBtn" class="flex items-center gap-2 p-1 pr-2 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors focus-ring">
                        ...
                        <div class="hidden sm:block text-left">
                            <p class="text-xs font-semibold text-ink-900 dark:text-white leading-tight">{{ auth()->user()->name }}</p>
                            <p class="text-[10px] text-ink-500 leading-tight">{{ ucfirst(auth()->user()->role ?? 'admin') }}</p>
                        </div>
                    ...
                    <div id="userDropdown" class="hidden absolute right-0 mt-2 w-64 bg-white dark:bg-ink-900 rounded-xl shadow-float border border-slate-200 dark:border-ink-800 overflow-hidden animate-scale-in origin-top-right">
                        <div class="p-4 border-b border-slate-100 dark:border-ink-800 bg-gradient-to-br from-brand-50 to-sphere-50 dark:from-ink-800 dark:to-ink-800">
                            <p class="text-sm font-bold text-ink-900 dark:text-white truncate">{{ auth()->user()->name }}</p>
                            <p class="text-xs text-ink-500 truncate mt-0.5">{{ auth()->user()->email }}</p>
                            <span class="inline-block mt-2 px-2 py-0.5 text-[10px] font-bold bg-brand-500 text-white rounded-full">
                                {{ ucfirst(auth()->user()->role ?? 'admin') }}
                            </span>
                        </div>
                        <div class="p-1.5">
                            <a href="{{ route('profile') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-ink-700 dark:text-ink-300 hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
                                <i class="bi bi-person text-[15px] text-ink-500"></i>
                                <span>My Profile</span>
                            </a>
                            <a href="{{ route('profile.datasets') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-ink-700 dark:text-ink-300 hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
                                <i class="bi bi-collection text-[15px] text-ink-500"></i>
                                <span>My Datasets</span>
                            </a>
                            <a href="{{ route('home') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-ink-700 dark:text-ink-300 hover:bg-slate-100 dark:hover:bg-ink-800 transition-colors">
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if ((e.metaKey || e.ctrlKey) && ['1','2','3','4'].includes(e.key)) {
                ...
                const routes = {
                    ...
                    '2': '{{ route("admin.datasets.index") }}',
                    '3': '{{ route("admin.posts.index") }}',
