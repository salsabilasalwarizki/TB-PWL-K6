@extends('layouts.app')
@section('title', 'My Profile - DataSphere Machine Learning Repository')
            ...
            <div class="flex flex-col md:flex-row items-center md:items-end gap-6">
                ...
                <div class="flex-1 text-center md:text-left">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 mb-3">
                        ...
                        <span class="text-xs font-semibold">{{ ucfirst($user->role ?? 'user') }}</span>
                    </div>
                    <h1 class="text-3xl md:text-4xl font-bold mb-1">{{ $user->name }}</h1>
                    ...
                    <p class="text-white/70 text-sm mt-1">
                        <i class="bi bi-building me-1"></i>{{ $user->institution }}
                    </p>
                    @endif
                </div>
                ...
                <div class="flex gap-4 md:gap-6">
                    ...
                    <div class="text-center">
                        <div class="text-2xl md:text-3xl font-bold">{{ number_format($user->datasets->sum('view_count') ?? 0) }}</div>
                        <div class="text-xs text-white/70 mt-1">Views</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl md:text-3xl font-bold">{{ number_format($user->datasets->sum('download_count') ?? 0) }}</div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <nav class="p-2">
                        <a href="{{ route('profile') }}" 
                           class="flex items-center gap-3 px-4 py-3 rounded-xl mb-1 transition-all {{ request()->routeIs('profile') ? 'bg-gradient-to-r from-brand-500 to-sphere-secondary text-white shadow-md' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50' }}">
                        ...
                        <a href="{{ route('profile.datasets') }}" 
                           class="flex items-center gap-3 px-4 py-3 rounded-xl mb-1 transition-all {{ request()->routeIs('profile.datasets*') ? 'bg-gradient-to-r from-brand-500 to-sphere-secondary text-white shadow-md' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50' }}">
                            <i class="bi bi-grid-3x3-gap-fill text-lg"></i>
                            <span class="font-semibold text-sm">My Datasets</span>
                            @if(request()->routeIs('profile.datasets*'))
                            <i class="bi bi-chevron-right ml-auto"></i>
                            @endif
                        </a>
                        <a href="{{ route('profile.edits') }}" 
                           class="flex items-center gap-3 px-4 py-3 rounded-xl mb-1 transition-all {{ request()->routeIs('profile.edits*') ? 'bg-gradient-to-r from-brand-500 to-sphere-secondary text-white shadow-md' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50' }}">
                            <i class="bi bi-pencil-fill text-lg"></i>
                            <span class="font-semibold text-sm">Edits</span>
                            @if(request()->routeIs('profile.edits*'))
                            <i class="bi bi-chevron-right ml-auto"></i>
                            @endif
                        </a>
                    </nav>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border-2 {{ $user->email_verified_at ? 'border-green-200 dark:border-green-800' : 'border-amber-200 dark:border-amber-800' }} overflow-hidden">
                    <div class="p-6 border-b {{ $user->email_verified_at ? 'border-green-100 dark:border-green-900/30' : 'border-amber-100 dark:border-amber-900/30' }}">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl {{ $user->email_verified_at ? 'bg-green-100 dark:bg-green-900/30' : 'bg-amber-100 dark:bg-amber-900/30' }} flex items-center justify-center">
                                <i class="bi {{ $user->email_verified_at ? 'bi-envelope-check-fill' : 'bi-envelope-exclamation' }} text-xl {{ $user->email_verified_at ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400' }}"></i>
                            </div>
                            <div>
                                <h2 class="text-xl font-bold text-gray-900 dark:text-white">Email Verification</h2>
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ $user->email_verified_at ? 'Your email is verified' : 'Verify your email to unlock all features' }}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                        Verified on {{ $user->email_verified_at->format('F d, Y \a\t g:i A') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                        We sent a verification link to <strong class="text-gray-900 dark:text-white">{{ $user->email }}</strong>. 
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div class="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700">
                                ...
                                <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold
                                    @if($user->role === 'admin' || $user->role === 'superadmin') bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400
                                    @elseif($user->role === 'contributor') bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:text-brand-400
                            ...
                            <div class="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700">
                                ...
                                <p class="text-gray-900 dark:text-white font-semibold">{{ $user->created_at->format('M d, Y') }}</p>
                            </div>
                            ...
                            <div class="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700 md:col-span-2">
                                ...
                                <p class="text-gray-900 dark:text-white font-semibold">{{ $user->institution ?? 'Not specified' }}</p>
                            </div>
                            ...
                            <div class="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700 md:col-span-2">
                                ...
                                <p class="text-gray-900 dark:text-white font-semibold">{{ $user->institution_address }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700">
                            <div class="flex items-center gap-3">
                                <i class="bi bi-envelope-check text-2xl {{ $user->email_verified_at ? 'text-green-500' : 'text-amber-500' }}"></i>
                                <div>
