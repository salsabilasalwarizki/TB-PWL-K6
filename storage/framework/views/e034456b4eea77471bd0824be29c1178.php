...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex gap-3">
            <a href="<?php echo e(route('admin.datasets.export')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-ink-700 dark:text-ink-300 bg-white dark:bg-ink-900 border border-ink-300 dark:border-ink-700 rounded-lg hover:bg-ink-50 dark:hover:bg-ink-800 hover:shadow-soft transition-all">
                <i class="bi bi-download"></i>
                <span>Export CSV</span>
            </a>
            <a href="<?php echo e(route('contribute.policy')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-brand-600 to-sphere-600 rounded-lg hover:shadow-lg hover:shadow-brand-500/30 transition-all">
        ...
        <div class="bg-white dark:bg-ink-900 rounded-xl shadow-card border border-ink-200 dark:border-ink-800 p-5 hover:-translate-y-0.5 hover:shadow-elev transition-all">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-ink-900 dark:text-white"><?php echo e($stats['total']); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-ink-900 rounded-xl shadow-card border border-ink-200 dark:border-ink-800 p-5 hover:-translate-y-0.5 hover:shadow-elev transition-all">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-ink-900 dark:text-white"><?php echo e($stats['pending']); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-ink-900 rounded-xl shadow-card border border-ink-200 dark:border-ink-800 p-5 hover:-translate-y-0.5 hover:shadow-elev transition-all">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-ink-900 dark:text-white"><?php echo e($stats['approved']); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-ink-900 rounded-xl shadow-card border border-ink-200 dark:border-ink-800 p-5 hover:-translate-y-0.5 hover:shadow-elev transition-all">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-ink-900 dark:text-white"><?php echo e($stats['rejected']); ?></p>
                </div>
            </div>
        </div>
    </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="sort" class="w-full px-4 py-2.5 rounded-lg bg-ink-50 dark:bg-ink-800 border border-ink-300 dark:border-ink-700 text-sm text-ink-900 dark:text-white focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all appearance-none cursor-pointer">
                    ...
                    <option value="download_count" <?php echo e(request('sort')=='download_count'?'selected':''); ?>>Most Downloaded</option>
                </select>
            </div>
    ...
    <div class="bg-white dark:bg-ink-900 rounded-xl shadow-card border border-ink-200 dark:border-ink-800 overflow-hidden">
        <form method="POST" action="<?php echo e(route('admin.datasets.bulk-action')); ?>" id="bulkForm">
            <?php echo csrf_field(); ?>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-ink-200 dark:divide-ink-800">
                        ...
                        <tr class="hover:bg-ink-50 dark:hover:bg-ink-800 transition-colors">
                            <td class="px-4 py-4">
                                <input type="checkbox" name="dataset_ids[]" value="<?php echo e($ds->dataset_id); ?>" class="w-4 h-4 rounded border-ink-300 dark:border-ink-600 text-brand-600 focus:ring-brand-500 dataset-checkbox">
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-3">
                                    <?php if($ds->thumbnail_url): ?>
                                    <img src="<?php echo e($ds->thumbnail_url); ?>" alt="" class="w-10 h-10 rounded-lg object-cover shadow-soft">
                                    <?php else: ?>
                                    ...
                                    <?php endif; ?>
                                    <div>
                                        <a href="<?php echo e(route('datasets.show', $ds)); ?>" class="font-semibold text-ink-900 dark:text-white hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                                            <?php echo e(Str::limit($ds->name, 40)); ?>

                                        </a>
                                        <?php if($ds->data_type): ?>
                                        <p class="text-xs text-ink-500 mt-0.5"><?php echo e($ds->data_type); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <span class="inline-flex px-2.5 py-1 text-xs font-medium text-ink-700 dark:text-ink-300 bg-ink-100 dark:bg-ink-800 rounded-md">
                                    <?php echo e($ds->subject_area ?? 'N/A'); ?>

                                </span>
                            </td>
                            <td class="px-4 py-4">
                                <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-md
                                    <?php echo e($ds->status==='approved' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : ''); ?>

                                    <?php echo e($ds->status==='available' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : ''); ?>

                                    <?php echo e($ds->status==='rejected' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : ''); ?>

                                    <?php echo e($ds->status==='pending' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' : ''); ?>
                            ...
                            <td class="px-4 py-4">
                                <span class="inline-flex items-center gap-1.5 text-sm text-ink-600 dark:text-ink-400">
                                    <i class="bi bi-eye"></i>
                                    <?php echo e(number_format($ds->view_count)); ?>

                                </span>
                            </td>
                            <td class="px-4 py-4">
                                <span class="text-sm text-ink-500"><?php echo e($ds->created_at->format('M d, Y')); ?></span>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-1">
                                    <a href="<?php echo e(route('admin.datasets.edit', $ds)); ?>" class="p-2 text-ink-600 dark:text-ink-400 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-ink-100 dark:hover:bg-ink-800 rounded-lg transition-colors" title="Edit">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <?php if($ds->status==='pending'): ?>
                                    <button type="button" onclick="quickAction('approve', <?php echo e($ds->dataset_id); ?>)" class="p-2 text-ink-600 dark:text-ink-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-ink-100 dark:hover:bg-ink-800 rounded-lg transition-colors" title="Approve">
                                        <i class="bi bi-check"></i>
                                    </button>
                                    <button type="button" onclick="quickAction('reject', <?php echo e($ds->dataset_id); ?>)" class="p-2 text-ink-600 dark:text-ink-400 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-ink-100 dark:hover:bg-ink-800 rounded-lg transition-colors" title="Reject">
                                        <i class="bi bi-x"></i>
                                    </button>
                                    <?php endif; ?>
                                    <button type="button" onclick="confirmDelete('<?php echo e(route('admin.datasets.destroy', $ds)); ?>')" class="p-2 text-ink-600 dark:text-ink-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-ink-100 dark:hover:bg-ink-800 rounded-lg transition-colors" title="Delete">
            ...
            <div class="px-4 py-3 bg-ink-50 dark:bg-ink-800 border-t border-ink-200 dark:border-ink-800 flex flex-wrap justify-between items-center gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-ink-500 ml-2"><?php echo e($datasets->total()); ?> items</span>
                </div>
...
function quickAction(action, id) {
    ...
    form.method = 'POST';
    form.action = `<?php echo e(url('admin/datasets')); ?>/${id}/${action}`;
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Documents\TB-PWL-DATASPHERE\resources\views/admin/datasets/index.blade.php ENDPATH**/ ?>